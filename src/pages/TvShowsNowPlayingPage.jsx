...
export default function TvShowsNowPlayingPage() {
  const [tvShows, setTvShows] = useState([]);
  const [page, setpage] = useState(1);

  useEffect(() => {
    async function getTvShowsNowPlaying() {
      const link = "https://api.themoviedb.org/3/tv/on_the_air?";
      const data = await TvShowList(link, page);
      setTvShows((prev) => [...prev, ...data]);
    }
    getTvShowsNowPlaying();
  }, [page]);
